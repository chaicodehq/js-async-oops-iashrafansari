...
export class NetworkError extends PaymentError {
  constructor(amount) {
   ...
   super("Network error during transaction", "NETWORK_ERROR");
   this.name = "NetworkError";
   this.retryable = true
...
export class FraudDetectedError extends PaymentError {
  constructor(amount) {
    // Your code here
     ...
     super("Suspicious transaction detected", "FRAUD_DETECTED");
     this.name = "FraudDetectedError";
     this.retryable = false;
  }
}
...
export async function processPayment(amount, balance, networkStatus) {
  ...
  if(amount <= 0) throw new PaymentError("Invalid amount", "INVALID_AMOUNT", amount);
  if(amount > balance) throw new InsufficientFundsError(amount, balance);
  if(networkStatus === "offline") throw new NetworkError(amount);
  if(amount > 100000) throw new FraudDetectedError(amount);
...
export async function retryPayment(paymentFn, maxRetries, delayMs) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // If retries exhausted → throw last NetworkError
...
export function categorizeError(error) {
  ...
  if(error instanceof InsufficientFundsError) return { type: "insufficient_funds", retryable: false, message: error.message };
  if(error instanceof NetworkError) return { type: "network", retryable: true, message: error.message };
  if(error instanceof FraudDetectedError) return { type: "fraud", retryable: false, message: error.message };
