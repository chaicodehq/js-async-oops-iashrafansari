...
export class HaveliSecurity {
  ...
  constructor(haveliName, passcode, maxResidents) {
    ...
    this.#passcode = passcode;
    this.#residents = [];  //array of resident objects
    this.#accessLog = [];
    this.#maxResidents = maxResidents;
  ...
  addResident(name, role, passcode) {
    // Your code here
    const roleList = ["malik", "naukar", "mehmaan"];

    if(passcode !== this.#passcode) return { success: false, message: "Galat passcode!" };

    if(!roleList.includes(role)) return { success: false, message: "Invalid role!" };

    const isMatch = this.#residents.filter(resident => resident.name === name)

    if(isMatch.length > 0) return {success: false, message: "Already a resident!" };

    if(this.#residents.length >= this.#maxResidents) return { success: false, message: "Haveli full hai!" };

    const resident = { name, role, addedAt: new Date().toISOString() };
  ...
  removeResident(name, passcode) {
    ...
    const removedResident = this.#residents.filter(resident => resident.name === name);

    if(removedResident.length === 0) return { success: false, message: "Resident nahi mila!" };

    const livingResident = this.#residents.filter(resident => resident.name !== name);
  ...
  changePasscode(oldPasscode, newPasscode) {
    ...
    if(!/^.{4,}$/.test(newPasscode)) return { success: false, message: "Naya passcode bahut chhota hai!" };

    this.#passcode = newPasscode;

    return { message: `Passcode badal diya!`, success: true};
